...
在终端中运行`git log`命令后，按下键盘上的`Q`键可以立即退出`git log`的分页显示，回到终端输入命令的状态。

使用分页命令：如果你想只显示部分提交历史记录，可以使用一些分页命令来控制显示的行数。例如，可以在`git log`命令后添加`-n`参数，其中`n`代表你想显示的提交记录数量。例如，`git log -5`将只显示最新的5条提交记录。当显示的记录数量不足以填满终端屏幕时，`git log`不会分页显示，而是直接显示全部记录。
